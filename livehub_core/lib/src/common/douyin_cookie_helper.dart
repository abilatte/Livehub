/// Pure Douyin cookie normalization helpers (no network).
///
/// Ported from the Simple Live fork so LiveHub can share the same cookie
/// completeness checks and browser-header paste handling.
class DouyinCookieHelper {
  ...
  static String cookieCompletenessHint(String cookie) {
    ...
    final hasDouyinIdentity = lower.contains('sessionid=') ||
  ...
  /// Normalize free-form user input into a Cookie header value.
  ///
  /// Accepts:
  /// - raw cookie string
  /// - `Cookie: ...` line
  /// - multi-line request headers containing a Cookie field
  /// - bare ttwid token without `ttwid=`
