...
import 'package:livehub_core/src/common/web_socket_util.dart';
...
class BiliBiliDanmakuUnavailable {
  final int roomId;
  final String reason;
  final int retryCount;

  BiliBiliDanmakuUnavailable({
    required this.roomId,
...
class BiliBiliDanmaku implements LiveDanmaku {
  ...
  bool _hasLoggedHeartbeatAck = false;
  bool _hasLoggedChatMessage = false;
  bool _hasLoggedAudienceMessage = false;
  @override
  Future start(dynamic args) async {
    if (args is BiliBiliDanmakuUnavailable) {
      CoreLog.w(
        "B站弹幕降级：roomId=${args.roomId} reason=${args.reason} retryCount=${args.retryCount}",
      );
      onClose?.call(args.reason);
      return;
    }
    if (args is! BiliBiliDanmakuArgs) {
      onClose?.call("弹幕参数读取失败，本次仅播放视频");
      return;
    }
    if (args.token.isEmpty || args.serverHost.isEmpty) {
      CoreLog.w(
        "B站弹幕参数无效：roomId=${args.roomId} host=${args.serverHost} tokenLength=${args.token.length}",
      );
      onClose?.call("弹幕参数读取失败，本次仅播放视频");
      return;
    }
    danmakuArgs = args;
    _hasLoggedHeartbeatAck = false;
    _hasLoggedChatMessage = false;
    _hasLoggedAudienceMessage = false;
    CoreLog.i(
      "B站弹幕开始连接：roomId=${danmakuArgs.roomId} host=${danmakuArgs.serverHost} tokenLength=${danmakuArgs.token.length} hasCookie=${danmakuArgs.cookie.isNotEmpty}",
    );
    webScoketUtils = WebScoketUtils(
      ...
      onReady: () {
        CoreLog.i("B站弹幕连接就绪：roomId=${danmakuArgs.roomId}");
        onReady?.call();
  ...
  void joinRoom(BiliBiliDanmakuArgs args) {
    CoreLog.i(
      "B站弹幕发送入房：roomId=${args.roomId} uid=${args.uid} buvidLength=${args.buvid.length}",
    );
  ...
  void decodeMessage(List<int> data) {
    try {
      ...
      if (operation == 3) {
        ...
        if (!_hasLoggedHeartbeatAck) {
          _hasLoggedHeartbeatAck = true;
          CoreLog.i("B站弹幕收到心跳回应：roomId=${danmakuArgs.roomId} online=$online");
        }
  ...
  void parseMessage(String jsonMessage) {
    try {
      ...
      if (cmd.contains("DANMU_MSG")) {
        if (!_hasLoggedChatMessage) {
          _hasLoggedChatMessage = true;
          CoreLog.i("B站弹幕收到首条聊天消息：roomId=${danmakuArgs.roomId}");
        }
        ...
        final audienceText = asT<String?>(data["online_count_text"]) ??
            asT<String?>(data["count_text"]) ??
            "";
        if (audienceText.isEmpty) {
          return;
        }
        if (!_hasLoggedAudienceMessage) {
          _hasLoggedAudienceMessage = true;
          CoreLog.i(
            "B站弹幕收到在线人数：roomId=${danmakuArgs.roomId} text=$audienceText",
          );
        }
        final audienceValue = int.tryParse(
          audienceText.replaceAll(RegExp(r"[^\d]"), ""),
        );
        onMessage?.call(
          LiveMessage(
            type: LiveMessageType.online,
