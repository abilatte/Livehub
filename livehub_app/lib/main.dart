...
import 'package:livehub_app/app/utils/listen_fourth_button.dart';
...
import 'package:livehub_app/modules/other/debug_log_page.dart';
import 'package:livehub_app/routes/app_pages.dart';
import 'package:livehub_app/routes/route_path.dart';
import 'package:livehub_app/services/bilibili_account_service.dart';
import 'package:livehub_app/services/douyin_account_service.dart';
...
import 'package:livehub_app/widgets/desktop_window_title_bar.dart';
...
Future initServices() async {
  ...
  Get.put(DesktopWindowController());
      ...
      return GetMaterialApp(
        ...
        builder: FlutterSmartDialog.init(
          ...
          builder: (context, child) {
            ...
            final desktopAdjustedMediaQueryData =
                fixedMediaQueryData.copyWith(
              viewPadding: fixedMediaQueryData.viewPadding.copyWith(
                top: 0,
              ),
              padding: fixedMediaQueryData.padding.copyWith(
                top: 0,
              ),
            );

            return MediaQuery(
              data: desktopAdjustedMediaQueryData,
              child: AppInteractionShell(child: child!),
            );
          },
        ),
      );
...
class AppInteractionShell extends StatefulWidget {
  final Widget child;

  const AppInteractionShell({
    required this.child,
    super.key,
  });

  @override
  State<AppInteractionShell> createState() => _AppInteractionShellState();
}

class _AppInteractionShellState extends State<AppInteractionShell> {
  late final FocusNode _keyboardFocusNode;

  @override
  void initState() {
    super.initState();
    _keyboardFocusNode = FocusNode(debugLabel: 'app_keyboard_listener');
  ...
  Future<bool> _exitDesktopFullScreenIfNeeded() async {
    final hasLiveRoomController = Get.isRegistered<LiveRoomController>();
    if (hasLiveRoomController) {
      final liveRoomController = Get.find<LiveRoomController>();
      if (liveRoomController.nativeFullScreenState.value ||
          await windowManager.isFullScreen()) {
        await liveRoomController.exitNativeFullScreen();
        return true;
      }
    }
        ...
        FourthButtonTapGestureRecognizer:
          ...
          (FourthButtonTapGestureRecognizer instance) {
            instance.onTapDown = (TapDownDetails details) async {
              if (await _exitDesktopFullScreenIfNeeded()) {
                return;
              }
              Get.back();
            };
      ...
      child: KeyboardListener(
        ...
        onKeyEvent: (KeyEvent event) async {
          if (event is KeyDownEvent &&
            ...
            await _exitDesktopFullScreenIfNeeded();
          }
        },
        child: widget.child,
      ),
        ...
        Column(
          children: [
            Obx(() {
              ...
              final showGlobalTitleBar =
                  desktopWindowController.showGlobalTitleBar &&
                      currentRoute != RoutePath.kLiveRoomDetail &&
                      !currentRoute.contains(RoutePath.kLiveRoomDetail);
              return Visibility(
                visible: showGlobalTitleBar,
                maintainState: true,
